...
class ConceptsAdapter:
    """
    Adapter for concepts dataset (e.g., concepts_v3_2_enriched).
    Expected input: data/processed/concepts/concepts_v3_2_enriched.jsonl.
    Output: LV0.7-aligned JSONL with manifest.
    """

    def run(self, *, input_dir: Path, output_path: Path, manifest_path: Path) -> AdapterResult:
        src_path = input_dir / "data" / "processed" / "concepts" / "concepts_v3_2_enriched.jsonl"
        ...
        with src_path.open("r", encoding="utf-8", errors="replace") as inp, output_path.open("w", encoding="utf-8") as out:
            for line in inp:
                ...
                stage = rec.get("stage") or "concept"
                script = rec.get("script") or "Latn"
                source = rec.get("source") or "concepts_v3_2_enriched"
