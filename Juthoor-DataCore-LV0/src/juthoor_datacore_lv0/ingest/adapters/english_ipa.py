...
class EnglishIPAAdapter:
    """
    Adapter for English IPA merged dataset.
    Expected input: data/processed/english/english_ipa_merged_pos.jsonl (from scripts/ingest).
    Output: LV0.7-aligned JSONL with manifest.
    """

    def run(self, *, input_dir: Path, output_path: Path, manifest_path: Path) -> AdapterResult:
        src_path = input_dir / "data" / "processed" / "english" / "english_ipa_merged_pos.jsonl"
        ...
        with src_path.open("r", encoding="utf-8", errors="replace") as inp, output_path.open("w", encoding="utf-8") as out:
            for line in inp:
                ...
                if not line:
                    continue
                rec = json.loads(line)
                lang = rec.get("language") or "eng"
                stage = rec.get("stage") or "modern"
                script = rec.get("script") or "Latn"
                source = rec.get("source") or "english-ipa-merged"
        ...
        write_manifest(
            ...
            generated_by="EnglishIPAAdapter",
